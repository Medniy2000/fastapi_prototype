...
from sqlalchemy import delete, exists, func, insert, select, Select, String, text, update
...
class AbstractBaseRepository(AbstractRepository, Generic[BaseEntityType]):
    MODEL: Optional[Type[Base]] = None
    ENTITY: Optional[Type[BaseEntity]] = None
    ...
    @classmethod
    async def get_first(cls, filter_data: dict) -> BaseEntityType | None:
        raise NotImplementedError

    @classmethod
    async def get_list(cls, filter_data: dict, order_data: Tuple[str] = ("id",)) -> List[BaseEntityType]:
        raise NotImplementedError

    @classmethod
    async def create(cls, data: dict, is_return_require: bool = False) -> BaseEntityType | None:
        raise NotImplementedError

    @classmethod
    async def create_bulk(cls, items: List[dict], is_return_require: bool = False) -> List[BaseEntityType] | None:
        raise NotImplementedError
    ...
    @classmethod
    async def update_bulk(cls, items: List[dict], is_return_require: bool = False) -> List[BaseEntityType] | None:
        raise NotImplementedError
...
class BaseSQLAsyncDrivenBaseRepository(AbstractBaseRepository[BaseEntityType], Generic[BaseEntityType]):
    MODEL: Optional[Type[Base]] = None
    ENTITY: Optional[Type[BaseEntity]] = None
    ...
    async def get_list(
        ...
        entity_ = cls.entity()
        items = [entity_(**i.__dict__) for i in raw_items]
        return items
